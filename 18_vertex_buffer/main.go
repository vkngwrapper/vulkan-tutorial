...
func (app *HelloTriangleApplication) createVertexBuffer() error {
	...
	app.vertexBuffer, _, err = app.device.CreateBuffer(app.allocator, &resource.BufferOptions{
	...
	memoryTypeIndex, err := app.findMemoryType(memRequirements.MemoryType, resource.MemoryHostVisible|resource.MemoryHostCoherent)
	if err != nil {
		return err
	}

	app.vertexBufferMemory, _, err = app.device.AllocateMemory(app.allocator, &resource.DeviceMemoryOptions{
